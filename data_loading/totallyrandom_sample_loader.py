...
class TotallyRandomSampleLoader:
    ...
    def load_sample(self, uni):
        ...
        if self.isTrain:
            ...
            dwi = self.load_totally_randomdwi(dir_name = 'dwi', uni = uni,  suffix = '_DWI_processed_b1000.mif.gz', random = True)
    ...
    #---------------------------------------------#
    #                Load Totally randomDWI       #
    #---------------------------------------------#
    # Load the data of a sample from the data set
    def load_totally_randomdwi(self, dir_name , uni , suffix, random = True):

        dwi_path = os.path.join(self.root, dir_name,  uni + suffix)
        dwi = self.interface.load_totallyrandom_downsampled_mif_dwi(dwi_path, index = uni)
